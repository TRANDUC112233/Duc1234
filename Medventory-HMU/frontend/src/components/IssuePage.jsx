...
  // Fetch dữ liệu ban đầu - Bỏ kiểm tra quyền ở đây
  useEffect(() => {
    fetchInitialData();
  }, []);
...
        // Giả định API Backend vẫn cần X-User-Id để lọc dữ liệu liên quan
...
          ? { 
                ...
                qtyIssued: Math.min(
                    qtyIssued, 
                    detail.qtyRequested, 
                    detail.selectedLot?.availableStock || detail.availableStock
                ) 
            }
          : detail
      )
...
              // Cập nhật lại số lượng xuất tối đa nếu chọn lô
...
      // Kiểm tra xem nếu chưa chọn lô, tồn tổng có đủ không
...
      }
      
      // Nếu có lô nhưng chưa chọn, cảnh báo người dùng (Có thể bỏ qua nếu cho phép xuất không chọn lô)
...
      return;
    }
    // Đã bỏ kiểm tra quyền: if (currentUser.roleCheck !== 2) 
...
  // LOẠI BỎ TOÀN BỘ KHỐI KIỂM TRA QUYỀN TRUY CẬP Ở ĐÂY
  // if (currentUser.roleCheck !== 2) { return (...) }
...
                                min="0"
                                // Cập nhật Max theo logic mới sau khi bỏ role check
...
                            <td>
                              {detail.selectedLot ? (
                                <span className="text-success">Đã chọn lô</span>
