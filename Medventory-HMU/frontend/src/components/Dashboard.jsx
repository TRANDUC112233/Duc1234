...
export default function Dashboard() {
    ...
    const roleDisplayMapping = {
        "lanhdao": "Lãnh đạo", // Vai trò Hiệu trưởng
        "thukho": "Thủ kho", 
        "canbo": "Cán bộ"
    };
    ...
    // LẤY THÔNG TIN USER KHI COMPONENT MOUNT (ĐÃ CHỈNH SỬA)
    useEffect(() => {
        // Dữ liệu người dùng mặc định là Hiệu trưởng
        const defaultUser = {
            id: 999,
            name: "Hiệu trưởng", // Tên hiển thị
            role: "lanhdao",    // Mã vai trò để mapping
            // Bạn có thể thêm các thuộc tính khác (ví dụ: token, permissions)
        };

        const savedUser = localStorage.getItem('currentUser');
        let userDataToUse = defaultUser; // Mặc định là Hiệu trưởng
        ...
        // Luôn sử dụng userDataToUse (hoặc từ LS, hoặc mặc định)
        if (userDataToUse) {
            // Chuyển đổi role từ backend enum sang tiếng Việt để hiển thị
            const formattedUserData = {
                ...userDataToUse,
                role: roleDisplayMapping[userDataToUse.role] || userDataToUse.role
            };
    ...
    // === THỐNG KÊ (DERIVED STATE) ===
    const total = equipmentData.length;
    const working = equipmentData.filter(e => e.status === "Hoạt động tốt").length;
    const maintenance = equipmentData.filter(e => e.status === "Cần bảo trì").length;
    const broken = equipmentData.filter(e => e.status === "Hỏng hóc").length;

    // === 4. JSX RETURN (GIAO DIỆN) ===
    return (
